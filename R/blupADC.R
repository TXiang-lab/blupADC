...
geno_qc_impute<-function(
	...
	input_data_blupf90_map=NULL,	 
	 ...
	 plink_software_path=system.file("extdata/bin", package = "blupADC"),
	 plink_software_name="plink", 
	 chr_set=NULL, # 识别多个染色体-plink, 最多可识别多个95个 	 
	 keep_inds_set=NULL,   #字符串向量
	 keep_snps_set=NULL,   #字符串向量
	 keep_chroms_set=NULL, #数值向量
	 beagle_software_path=system.file("extdata/bin", package = "blupADC"),
...
cal_kinship<-function(
		...
		miss_base_num=0,		
	     ...
	     gene_dropping=FALSE,
	     gene_dropping_iteration=1000, # gene_dropping 的模拟次数
...
if(is.data.frame(input_data_numeric)){input_data_numeric=DataFrame_to_arma(input_data_numeric)}
...
G_A=NULL;G_Ainv=NULL;G_D=NULL;G_Dinv=NULL;P_A=NULL;P_Ainv=NULL;P_D=NULL;P_Dinv=NULL;H_A=NULL;H_Ainv=NULL
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(P_D,IND_geno=s.numeric(IND_pedigree),det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(P_Dinv,IND_geno=s.numeric(IND_pedigree),det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(G_A,IND_geno=IND_geno,det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(G_Ainv,IND_geno=IND_geno,det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(G_D,IND_geno=IND_geno,det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(G_Dinv,IND_geno=IND_geno,det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
H_Dresult=makeHA_cpp(num_ped,input_data_numeric,IND_geno,pos_A11,pos_A22,pos_geno,pos_A,pos_H22,APY_algorithm,APY_eigen_threshold,APY_n_core,direct=TRUE,inverse=TRUE,omega=SSBLUP_omega)
H_D=H_Dresult$H
H_Dinv=NULL
rm(H_Dresult);gc();
...
H_Dinv=makeHA_cpp(num_ped,input_data_numeric,IND_geno,pos_A11,pos_A22,pos_geno,pos_A,pos_H22,APY_algorithm,APY_eigen_threshold,APY_n_core,direct=FALSE,inverse=TRUE,omega=SSBLUP_omega)$Hinv
H_D=NULL
}
}else{

if("H_D"%in%kinship_type&(!"H_Dinv"%in%kinship_type)){
H_Dresult=makeHA_metafounder_cpp(num_ped,input_data_numeric,pos_A11,pos_A22,pos_geno,pos_A,pos_H22,direct=TRUE,inverse=FALSE,omega=SSBLUP_omega)
H_D=H_Dresult$H
H_Dinv=NULL
rm(H_Dresult);gc();
}else if("H_D"%in%kinship_type&("H_Dinv"%in%kinship_type)){
H_Dresult=makeHA_metafounder_cpp(num_ped,input_data_numeric,pos_A11,pos_A22,pos_geno,pos_A,pos_H22,direct=TRUE,inverse=TRUE,omega=SSBLUP_omega)
H_D=H_Dresult$H
H_Dinv=H_Dresult$Hinv
rm(H_Dresult);gc();
}else if(!"H_D"%in%kinship_type&("H_Dinv"%in%kinship_type)){
H_Dinv=makeHA_metafounder_cpp(num_ped,input_data_numeric,pos_A11,pos_A22,pos_geno,pos_A,pos_H22,direct=FALSE,inverse=TRUE,omega=SSBLUP_omega)$Hinv
H_D=NULL
}
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(H_A,IND_geno=IND_Additive,det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if("col_three" %in% output_matrix_type){
O0O0OOO0O0OOOOOOOOOO=matrix_col3(H_Ainv,IND_geno=IND_Additive,det=matrix_log_det,cpu_cores=cpu_cores,threshold=col3_threshold) 
...
if(NA%in%as.numeric(OOOOO0OOOOOOOOO0OOOO[,2])){
...
run_BLUPF90<-function(
								   ...
								   BLUPF90_map_name=NULL,
								   plot_variance=FALSE,
								   BLUPF90_software_path=system.file("extdata/bin", package = "blupADC")
...
t_pos=(1:nrow(renf90))[renf90[,1]%in%"add_animal"]
renf90.phe=read.table("renf90.dat",header=F)
for(OOO0OOO0O0OOO0O0OOO0OOOO in 1:length(t_pos)){
pos=t_pos[OOO0OOO0O0OOO0O0OOO0OOOO]
renf90[pos,1]="user_file"
new_ped_name=renf90[pos+2,1]
renf90[pos+2,1]=relationship_name[OOO0OOO0O0OOO0O0OOO0OOOO]
renf90[is.na(renf90)]=""
#将表型数据中的new_id 替换成原始id
renf90.ped=read.table(new_ped_name,header=F)
new_id_pos=ncol(renf90.phe)+1-OOO0OOO0O0OOO0O0OOO0OOOO
renf90.phe[,new_id_pos]=renf90.ped[match(renf90.phe[,new_id_pos],renf90.ped[,1]),ncol(renf90.ped)]
}
...
#计算校正表型
O0O0OOO0O0OOOOO0O0OO=cal_corrected_phe_BLUPF90(relationship_name=relationship_name,analysis_model=analysis_model,target_trait_name=target_trait_name)
...
cal_corrected_phe_BLUPF90<-function(relationship_name=NULL,analysis_model=NULL,target_trait_name=NULL){
...
if(analysis_model=="User_define"){
max_effect_number=(max_effect_number+1-length(relationship_name)):max_effect_number
}
...
O0O0OOO0O0OOOOO0O0OO=matrix(NA,nrow=length(unique(ebv[,"Id"])),ncol=length(max_effect_number)+4*length(target_trait_name))
O0O0OOO0O0OOOOO0O0OO=as.data.frame(O0O0OOO0O0OOOOO0O0OO,stringsAsFactors=F)
...
tmp_ebv=ebv[ebv$Effect_number%in%max_effect_number[OOO0OOO0O0OOO0O0OOO0OOOOO],]
trait_ebv=tmp_ebv[tmp_ebv$OOOOO0O0O0O0OOOOOOO0%in%OOO0OOO0O0OOO0O0OOO0OOOO,]

O0O0OOO0O0OOOOO0O0OO[,(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+1]=trait_ebv[match(O0O0OOO0O0OOOOO0O0OO[,1],tmp_ebv[,"Id"]),"EBV"]
O0O0OOO0O0OOOOO0O0OO[,(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+2]=residual[match(O0O0OOO0O0OOOOO0O0OO[,1],residual[,"Id"]),length(target_trait_name)+OOO0OOO0O0OOO0O0OOO0OOOO]
O0O0OOO0O0OOOOO0O0OO[,(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+3]=O0O0OOO0O0OOOOO0O0OO[,(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+2]+O0O0OOO0O0OOOOO0O0OO[,(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+1]
colnames(O0O0OOO0O0OOOOO0O0OO)[(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+1]=paste0(target_trait_name[OOO0OOO0O0OOO0O0OOO0OOOO],"_R",OOO0OOO0O0OOO0O0OOO0OOOOO,"_Value")
colnames(O0O0OOO0O0OOOOO0O0OO)[(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+2]=paste0(target_trait_name[OOO0OOO0O0OOO0O0OOO0OOOO],"_Residual")
colnames(O0O0OOO0O0OOOOO0O0OO)[(OOO0OOO0O0OOO0O0OOO0OOOO-1)*(length(relationship_name)+2)+OOO0OOO0O0OOO0O0OOO0OOOOO+3]=paste0(target_trait_name[OOO0OOO0O0OOO0O0OOO0OOOO],"_EBV_Plus_Residual")
...
run_DMU<-function(
					   ...
					   output_ebv_path=NULL,
					   output_ebv_name=NULL,			   
					   DMU_software_path=system.file("extdata/bin", package = "blupADC"),
					   IND_geno_file_name="IND_geno.txt", #
					   IND_geno=NULL,
					   SSBLUP_omega=0.05,
					   plot_variance=FALSE,
